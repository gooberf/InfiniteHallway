...
def start():
    try:
        ...
        inventory = []
        quickStart = False
        if os.path.exists('config/quickStart'):
            with open('config/quickStart', 'r') as f:
                data = f.read()
                if data == "1":
                    quickStart = True
        ...
        if not quickStart:
            ...
            if not os.path.exists("config/easyMode"):
                ...
                if easy_status == "1":
                    ...
                    pass
        if not os.path.exists('config/quickStart'):
            quick = choose.two_options("Would you like to jump to the menu immediately with starting the game?", "yes", "no")
            with open('config/quickStart', 'w') as f:
                if quick == "yes":
                    f.write("1")
                else:
                    f.write("0")
        ...
        if os.name == 'nt':
            ...
            os.system('clear')
        mh.scan()
        mainMenuOption = choose.three_options("main menu placeholder", "start", "reset config", "load mod")
